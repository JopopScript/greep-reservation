...
from app.docs.error_responses import (
...
async def get_schedules(
    ...
    schedule_page: SchedulePage = await service.list(
        ScheduleQuery(
            page_number=page, page_size=page_size, account_id=account_id(request)
        )
    )
...
@router.post(
    "/schedules", response_model=ScheduleResponse, responses=customer_create_schedule
)
...
async def change_schedule(
    ...
    schedule = await service.customer_update(
        schedule_id, schedule_request.to_form(), account_id(request)
    )
    return ScheduleResponse.from_schedule(schedule)
