...
class ScheduleResponse(BaseModel):
    ...
    @staticmethod
    def fromSchedule(schedule: Schedule) -> 'ScheduleResponse':
        return ScheduleResponse(
            ...
            profile=ProfileResponse.fromAccount(schedule.account),
        )
...
class ScheduleRequest(BaseModel):
    ...
    def must_be_hourly(cls, d: datetime):
        if d.minute != 0 or d.second != 0 or d.microsecond != 0:
            raise BusinessException('Time must be set at the beginning of an hour (e.g., 12:00:00).',
                                    ErrorCode.INVALID_ARGUMENT)
        return d
    ...
    def validate_start_end_times(cls, request):
        ...
        if start_at >= end_at:
            raise BusinessException('start time must be before end time.', ErrorCode.INVALID_ARGUMENT)
        return request
...
class CustomerScheduleCancelRequest(BaseModel):
    ...
    def must_be_cancel(cls, status: ScheduleStatus):
        if status != ScheduleStatus.CANCELED:
            raise BusinessException('change status must be CANCELED.', ErrorCode.INVALID_ARGUMENT)
        return status
...
class AdminScheduleStatusChangeRequest(BaseModel):
    ...
    def must_be_cancel(cls, status: ScheduleStatus):
        if status == ScheduleStatus.PENDING:
            raise BusinessException('change status must not be PENDING.', ErrorCode.INVALID_ARGUMENT)
        return status
