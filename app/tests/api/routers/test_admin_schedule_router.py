...
from app.tests.fixture_util import pending_schedule, canceled_schedule, confirmed_schedule, datetime_to_str
...
@pytest.mark.anyio
async def test_admin_schedule_pending_to_confirmed(client: AsyncClient, tokens: Tokens) -> None:
    start_at = datetime(2100, 1, 1, 0, 0, 0)
    schedule_id = await pending_schedule(client, tokens.first_token(), start_at, 50_000)
...
@pytest.mark.anyio
async def test_admin_schedule_canceled_to_confirmed(client: AsyncClient, tokens: Tokens) -> None:
    start_at = datetime(2100, 1, 1, 0, 0, 0)
...
@pytest.mark.anyio
async def test_admin_schedule_pending_to_canceled(client: AsyncClient, tokens: Tokens) -> None:
    start_at = datetime(2100, 1, 1, 0, 0, 0)
    schedule_id = await pending_schedule(client, tokens.first_token(), start_at, 50_000)
...
@pytest.mark.anyio
async def test_admin_schedule_confirmed_to_canceled(client: AsyncClient, tokens: Tokens) -> None:
    start_at = datetime(2100, 1, 1, 0, 0, 0)
