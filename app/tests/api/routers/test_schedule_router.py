...
async def test_customer_get_own_schedule(client: AsyncClient, tokens: Tokens) -> None:
    ...
    response = await client.get(
        "/schedules", headers=tokens.first_token(), params={"page": 0, "page-size": 5}
    )
...
async def test_create_schedule_success(client: AsyncClient, tokens: Tokens) -> None:
    ...
    response = await client.post("/schedules", headers=tokens.first_token(), json=body)
...
@pytest.mark.anyio
async def test_create_schedule_before_3days(
...
@pytest.mark.anyio
async def test_create_schedule_50k_applicant(
    client: AsyncClient, tokens: Tokens
) -> None:
    body = schedule_create_request(datetime(2103, 1, 1, 0, 0, 0), 50_000)
    response = await client.post("/schedules", headers=tokens.first_token(), json=body)
...
@pytest.mark.anyio
async def test_create_schedule_decreased_applicant_over(
    client: AsyncClient, tokens: Tokens
) -> None:
...
@pytest.mark.anyio
async def test_create_schedule_decreased_applicant_under(
    client: AsyncClient, tokens: Tokens
) -> None:
    start_at = datetime(2104, 1, 1, 0, 0, 0)
    await confirmed_schedule(client, tokens, start_at, 10_000)
...
@pytest.mark.anyio
async def test_create_multi_pending_schedule(
    client: AsyncClient, tokens: Tokens
) -> None:
    start_at = datetime(2104, 1, 1, 0, 0, 0)
    await pending_schedule(client, tokens.first_token(), start_at, 50_000)
...
async def test_change_schedule(client: AsyncClient, tokens: Tokens) -> None:
    ...
    response = await client.put(
        f"/schedules/{schedule_id}", headers=tokens.first_token(), json=body
    )
